...
async function main() {
  ...
  // Clear existing BOLA findings for this domain to avoid duplicates in testing
  const deletedCount = await Vulnerability.deleteMany({
    domainId: domain._id,
    category: 'API Security',
    subCategory: 'Broken Object Level Authorization (BOLA)'
  });
  console.log(`Cleared ${deletedCount.deletedCount} old BOLA vulnerabilities`);
  ...
  const scan = await Scan.create({
    ...
    authProfileId: profileA._id, // Assign User A as primary credentials
    checks: {
      ssl: false,
      headers: false,
      owasp: false,
  ...
  console.log('\n========================================');
  console.log('BOLA SCAN COMPLETED');
  console.log('========================================');
...
main()
  .catch((error) => {
    console.error(`BOLA test run failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
