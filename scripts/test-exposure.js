...
async function main() {
  ...
  // Clear existing Excessive Data Exposure findings for this domain to avoid duplicates in testing
  const deletedCount = await Vulnerability.deleteMany({
    domainId: domain._id,
    category: 'API Security',
    subCategory: 'Excessive Data Exposure'
  });
  console.log(`Cleared ${deletedCount.deletedCount} old Excessive Data Exposure vulnerabilities`);
  ...
  const savedFindings = await Vulnerability.find({ scanId: scan._id, subCategory: 'Excessive Data Exposure' });

  console.log('\n========================================');
  console.log('EXCESSIVE DATA EXPOSURE SCAN COMPLETED');
  console.log('========================================');
...
main()
  .catch((error) => {
    console.error(`Excessive Data Exposure test run failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
