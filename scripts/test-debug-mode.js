...
async function main() {
  ...
  const deletedCount = await Vulnerability.deleteMany({
    ...
    subCategory: 'Debug Mode Enabled'
  });
  console.log(`Cleared ${deletedCount.deletedCount} old Debug Mode Enabled findings`);
  ...
  console.log('\n========================================');
  console.log('DEBUG MODE SCAN COMPLETED');
  console.log('========================================');
  ...
  const hasVuln = findings.some(f => f.path === '/api/test-debug-mode/vulnerable/trace');
  if (!hasVuln) {
    throw new Error('Test failed: exposed debug mode trace was not detected.');
  }
  console.log('\nPositive Case Check: PASSED');

  // Verify negative case
  const hasSecure = findings.some(f => f.path === '/api/test-debug-mode/secure/trace');
  if (hasSecure) {
    throw new Error('Test failed: false positive generated for secure error trace.');
  }
  ...
  const totalOpenInDb = await Vulnerability.countDocuments({
    domainId: domain._id,
    path: '/api/test-debug-mode/vulnerable/trace',
    subCategory: 'Debug Mode Enabled',
    status: { $ne: 'Resolved' }
  });

  console.log(`Total open findings in DB for trace path: ${totalOpenInDb}`);
  if (totalOpenInDb !== 1) {
  ...
  console.log('\n========================================');
  console.log('ALL DEBUG MODE TEST VERIFICATIONS PASSED SUCCESSFULLY!');
  console.log('========================================');
}

main()
  .catch((error) => {
    console.error(`\nDebug Mode tests failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
