...
async function main() {
  ...
  // Clear existing Path Traversal findings for this domain to avoid duplicates in testing
  const deletedCount = await Vulnerability.deleteMany({
    domainId: domain._id,
    category: 'File & Upload',
    subCategory: 'Path Traversal'
  });
  console.log(`Cleared ${deletedCount.deletedCount} old Path Traversal vulnerabilities`);
  ...
  const scan = await Scan.create({
    ...
    checks: {
      ...
      fileUpload: true // This will enable both fileUpload and pathTraversal scanners
    }
  });
  ...
  const savedFindings = await Vulnerability.find({ scanId: scan._id, subCategory: 'Path Traversal' });

  console.log('\n========================================');
  console.log('PATH TRAVERSAL SCAN COMPLETED');
  console.log('========================================');
...
main()
  .catch((error) => {
    console.error(`Path Traversal test run failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
