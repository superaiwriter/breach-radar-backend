...
async function main() {
  ...
  // Clear existing Mass Assignment findings for this domain to avoid duplicates in testing
  const deletedCount = await Vulnerability.deleteMany({
    domainId: domain._id,
    category: 'API Security',
    subCategory: 'Mass Assignment'
  });
  console.log(`Cleared ${deletedCount.deletedCount} old Mass Assignment vulnerabilities`);
  ...
  const savedFindings = await Vulnerability.find({ scanId: scan._id, subCategory: 'Mass Assignment' });

  console.log('\n========================================');
  console.log('MASS ASSIGNMENT SCAN COMPLETED');
  console.log('========================================');
...
main()
  .catch((error) => {
    console.error(`Mass Assignment test run failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
