...
async function main() {
  ...
  // Clear existing XSS findings for this domain to avoid duplicates in testing
  const deletedCount = await Vulnerability.deleteMany({
    domainId: domain._id,
    category: 'Client Side',
    subCategory: 'Cross-Site Scripting (XSS)'
  });
  console.log(`Cleared ${deletedCount.deletedCount} old XSS vulnerabilities`);
  ...
  const scan = await Scan.create({
    ...
    checks: {
      ...
      fileUpload: false,
      injection: false,
      clientSide: true // Enables XSS scan
    }
  });
  ...
  const savedFindings = await Vulnerability.find({ scanId: scan._id, subCategory: 'Cross-Site Scripting (XSS)' });

  console.log('\n========================================');
  console.log('XSS SCAN COMPLETED');
  console.log('========================================');
...
main()
  .catch((error) => {
    console.error(`XSS test run failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
