...
async function main() {
  ...
  // Clear existing Exposed Admin Panels findings for this domain to avoid duplicates in testing
  const deletedCount = await Vulnerability.deleteMany({
    domainId: domain._id,
    category: 'Cloud & Infrastructure',
    subCategory: 'Exposed Admin Panels'
  });
  console.log(`Cleared ${deletedCount.deletedCount} old Exposed Admin Panels vulnerabilities`);
  ...
  const savedFindings = await Vulnerability.find({ scanId: scan._id, subCategory: 'Exposed Admin Panels' });

  console.log('\n========================================');
  console.log('EXPOSED ADMIN PANELS SCAN COMPLETED');
  console.log('========================================');
...
main()
  .catch((error) => {
    console.error(`Exposed Admin Panels test run failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
