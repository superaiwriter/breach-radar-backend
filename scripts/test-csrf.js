...
async function main() {
  ...
  // Clear existing CSRF findings for this domain to avoid duplicates in testing
  const deletedCount = await Vulnerability.deleteMany({
    domainId: domain._id,
    category: 'Client Side',
    subCategory: 'Cross-Site Request Forgery (CSRF)'
  });
  console.log(`Cleared ${deletedCount.deletedCount} old CSRF vulnerabilities`);
  ...
  const scan = await Scan.create({
    ...
    checks: {
      ...
      fileUpload: false,
      injection: false,
      clientSide: true // Enables XSS & CSRF scan
    }
  });
  ...
  const savedFindings = await Vulnerability.find({ scanId: scan._id, subCategory: 'Cross-Site Request Forgery (CSRF)' });
...
main()
  .catch((error) => {
    console.error(`CSRF test run failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
