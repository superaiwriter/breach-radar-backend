...
async function main() {
  ...
  // Clear existing Insecure File Download findings for this domain to avoid duplicates in testing
  const deletedCount = await Vulnerability.deleteMany({
    domainId: domain._id,
    category: 'File & Upload',
    subCategory: 'Insecure File Download'
  });
  console.log(`Cleared ${deletedCount.deletedCount} old Insecure File Download vulnerabilities`);
  ...
  const scan = await Scan.create({
    ...
    checks: {
      ...
      fileUpload: true // Enabes both fileUpload, pathTraversal, and insecureFileDownload
    }
  });
  ...
  const savedFindings = await Vulnerability.find({ scanId: scan._id, subCategory: 'Insecure File Download' });

  console.log('\n========================================');
  console.log('INSECURE FILE DOWNLOAD SCAN COMPLETED');
  console.log('========================================');
...
main()
  .catch((error) => {
    console.error(`Insecure File Download test run failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
