...
async function main() {
  ...
  // Clear existing SQL Injection findings for this domain to avoid duplicates in testing
  const deletedCount = await Vulnerability.deleteMany({
    domainId: domain._id,
    category: 'Injection',
    subCategory: 'SQL Injection'
  });
  console.log(`Cleared ${deletedCount.deletedCount} old SQL Injection vulnerabilities`);
  ...
  const scan = await Scan.create({
    ...
    checks: {
      ...
      fileUpload: false,
      injection: true // Enabes sqlInjection scanner
    }
  });
  ...
  const savedFindings = await Vulnerability.find({ scanId: scan._id, subCategory: 'SQL Injection' });

  console.log('\n========================================');
  console.log('SQL INJECTION SCAN COMPLETED');
  console.log('========================================');
  ...
  savedFindings.forEach((finding, idx) => {
    ...
    console.log(`- Parameter: ${finding.parameter}`);
...
main()
  .catch((error) => {
    console.error(`SQL Injection test run failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
