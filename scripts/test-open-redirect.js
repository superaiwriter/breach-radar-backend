...
async function main() {
  ...
  // Clear existing Open Redirect findings for this domain to avoid duplicates in testing
  const deletedCount = await Vulnerability.deleteMany({
    domainId: domain._id,
    category: 'Client Side',
    subCategory: 'Open Redirect'
  });
  console.log(`Cleared ${deletedCount.deletedCount} old Open Redirect vulnerabilities`);
  ...
  const scan = await Scan.create({
    ...
    checks: {
      ...
      fileUpload: false,
      injection: false,
      clientSide: true // Enables XSS, CSRF, and Open Redirect scans
    }
  });
  ...
  const savedFindings = await Vulnerability.find({ scanId: scan._id, subCategory: 'Open Redirect' });

  console.log('\n========================================');
  console.log('OPEN REDIRECT SCAN COMPLETED');
  console.log('========================================');
...
main()
  .catch((error) => {
    console.error(`Open Redirect test run failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
