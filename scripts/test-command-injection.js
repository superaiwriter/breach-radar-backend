...
async function main() {
  ...
  // Clear existing Command Injection findings for this domain to avoid duplicates in testing
  const deletedCount = await Vulnerability.deleteMany({
    domainId: domain._id,
    category: 'Injection',
    subCategory: 'Command Injection'
  });
  console.log(`Cleared ${deletedCount.deletedCount} old Command Injection vulnerabilities`);
  ...
  const scan = await Scan.create({
    ...
    checks: {
      ...
      fileUpload: false,
      injection: true // Enables injection check (runs sqlInjection, noSqlInjection, and commandInjection)
    }
  });
  ...
  const savedFindings = await Vulnerability.find({ scanId: scan._id, subCategory: 'Command Injection' });

  console.log('\n========================================');
  console.log('COMMAND INJECTION SCAN COMPLETED');
  console.log('========================================');
...
main()
  .catch((error) => {
    console.error(`Command Injection test run failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
