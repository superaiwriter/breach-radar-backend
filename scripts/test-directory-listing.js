...
async function main() {
  ...
  console.log(`Cleared ${deletedCount.deletedCount} old Directory Listing findings`);
  ...
  console.log('\n========================================');
  console.log('DIRECTORY LISTING SCAN COMPLETED');
  console.log('========================================');
  ...
  const hasVuln = findings.some(f => f.path === '/api/test-directory-listing/vulnerable/uploads/');
  if (!hasVuln) {
    throw new Error('Test failed: vulnerable directory listing was not detected.');
  }
  console.log('\nPositive Case Check: PASSED');

  // Verify negative case
  const hasSecure = findings.some(f => f.path === '/api/test-directory-listing/secure/uploads/');
  if (hasSecure) {
    throw new Error('Test failed: false positive generated for secure directory listing.');
  }
  ...
  console.log('\n========================================');
  console.log('ALL DIRECTORY LISTING TEST VERIFICATIONS PASSED SUCCESSFULLY!');
  console.log('========================================');
}

main()
  .catch((error) => {
    console.error(`\nDirectory Listing tests failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
