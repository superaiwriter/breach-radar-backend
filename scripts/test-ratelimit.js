...
async function main() {
  ...
  // Clear existing API Rate Limit Issues findings for this domain to avoid duplicates in testing
  const deletedCount = await Vulnerability.deleteMany({
    domainId: domain._id,
    category: 'API Security',
    subCategory: 'API Rate Limit Issues'
  });
  console.log(`Cleared ${deletedCount.deletedCount} old API Rate Limit Issues vulnerabilities`);
  ...
  const savedFindings = await Vulnerability.find({ scanId: scan._id, subCategory: 'API Rate Limit Issues' });

  console.log('\n========================================');
  console.log('RATE LIMIT SCAN COMPLETED');
  console.log('========================================');
...
main()
  .catch((error) => {
    console.error(`Rate Limit test run failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
