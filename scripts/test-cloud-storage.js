...
async function main() {
  ...
  // Clear existing Public Cloud Storage Exposure findings for this domain to avoid duplicates in testing
  const deletedCount = await Vulnerability.deleteMany({
    domainId: domain._id,
    category: 'Cloud & Infrastructure',
    subCategory: 'Public Cloud Storage Exposure'
  });
  console.log(`Cleared ${deletedCount.deletedCount} old Public Cloud Storage Exposure vulnerabilities`);
  ...
  const savedFindings = await Vulnerability.find({ scanId: scan._id, subCategory: 'Public Cloud Storage Exposure' });

  console.log('\n========================================');
  console.log('CLOUD STORAGE SCAN COMPLETED');
  console.log('========================================');
...
main()
  .catch((error) => {
    console.error(`Cloud Storage test run failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
