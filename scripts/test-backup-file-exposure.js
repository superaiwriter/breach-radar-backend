...
async function main() {
  ...
  const deletedCount = await Vulnerability.deleteMany({
    ...
    subCategory: 'Backup File Exposure'
  });
  console.log(`Cleared ${deletedCount.deletedCount} old Backup File Exposure findings`);
  ...
  console.log('\n========================================');
  console.log('BACKUP FILE EXPOSURE SCAN COMPLETED');
  console.log('========================================');
  ...
  const hasVuln = findings.some(f => f.path === '/api/test-backup-file-exposure/vulnerable/backup.sql');
  if (!hasVuln) {
    throw new Error('Test failed: exposed backup file was not detected.');
  }
  console.log('\nPositive Case Check: PASSED');

  // Verify negative case
  const hasSecure = findings.some(f => f.path === '/api/test-backup-file-exposure/secure/backup.sql');
  if (hasSecure) {
    throw new Error('Test failed: false positive generated for secure backup file.');
  }
  ...
  const duplicateScan = await Scan.create({
    ...
    triggeredBy: 'system-test',
    authProfileId: profileA._id,
    checks: { backupFileExposure: true }
  });
  ...
  console.log('\n========================================');
  console.log('ALL BACKUP FILE EXPOSURE TEST VERIFICATIONS PASSED SUCCESSFULLY!');
  console.log('========================================');
}

main()
  .catch((error) => {
    console.error(`\nBackup File Exposure tests failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
