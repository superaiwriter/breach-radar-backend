...
async function main() {
  ...
  // Clear existing SSTI findings for this domain to avoid duplicates in testing
  const deletedCount = await Vulnerability.deleteMany({
    domainId: domain._id,
    category: 'Injection',
    subCategory: 'SSTI'
  });
  console.log(`Cleared ${deletedCount.deletedCount} old SSTI vulnerabilities`);
  ...
  const scan = await Scan.create({
    ...
    checks: {
      ...
      fileUpload: false,
      injection: true // Enables injection check (runs sqlInjection, noSqlInjection, commandInjection, and ssti)
    }
  });
  ...
  const savedFindings = await Vulnerability.find({ scanId: scan._id, subCategory: 'SSTI' });

  console.log('\n========================================');
  console.log('SSTI SCAN COMPLETED');
  console.log('========================================');
...
main()
  .catch((error) => {
    console.error(`SSTI test run failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
