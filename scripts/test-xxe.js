...
async function main() {
  ...
  // Clear existing XXE findings for this domain to avoid duplicates in testing
  const deletedCount = await Vulnerability.deleteMany({
    domainId: domain._id,
    category: 'Injection',
    subCategory: 'XXE'
  });
  console.log(`Cleared ${deletedCount.deletedCount} old XXE vulnerabilities`);
  ...
  const scan = await Scan.create({
    ...
    checks: {
      ...
      fileUpload: false,
      injection: true // Enables injection check (runs sqlInjection, noSqlInjection, commandInjection, ssti, and xxe)
    }
  });
  ...
  const savedFindings = await Vulnerability.find({ scanId: scan._id, subCategory: 'XXE' });

  console.log('\n========================================');
  console.log('XXE SCAN COMPLETED');
  console.log('========================================');
...
main()
  .catch((error) => {
    console.error(`XXE test run failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
