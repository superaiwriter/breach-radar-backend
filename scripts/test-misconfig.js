...
async function main() {
  ...
  // Clear existing Security Misconfiguration findings for this domain to avoid duplicates in testing
  const deletedCount = await Vulnerability.deleteMany({
    domainId: domain._id,
    category: 'Cloud & Infrastructure',
    subCategory: 'Security Misconfiguration'
  });
  console.log(`Cleared ${deletedCount.deletedCount} old Security Misconfiguration vulnerabilities`);
  ...
  const savedFindings = await Vulnerability.find({ scanId: scan._id, subCategory: 'Security Misconfiguration' });

  console.log('\n========================================');
  console.log('SECURITY MISCONFIGURATION SCAN COMPLETED');
  console.log('========================================');
...
main()
  .catch((error) => {
    console.error(`Security Misconfiguration test run failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
