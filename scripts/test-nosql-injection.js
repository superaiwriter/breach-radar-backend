...
async function main() {
  ...
  // Clear existing NoSQL Injection findings for this domain to avoid duplicates in testing
  const deletedCount = await Vulnerability.deleteMany({
    domainId: domain._id,
    category: 'Injection',
    subCategory: 'NoSQL Injection'
  });
  console.log(`Cleared ${deletedCount.deletedCount} old NoSQL Injection vulnerabilities`);
  ...
  const scan = await Scan.create({
    ...
    checks: {
      ...
      fileUpload: false,
      injection: true // Enables both sqlInjection and noSqlInjection scanners
    }
  });
  ...
  const savedFindings = await Vulnerability.find({ scanId: scan._id, subCategory: 'NoSQL Injection' });

  console.log('\n========================================');
  console.log('NOSQL INJECTION SCAN COMPLETED');
  console.log('========================================');
...
main()
  .catch((error) => {
    console.error(`NoSQL Injection test run failed: ${error.message}`);
    process.exitCode = 1;
  })
  .finally(async () => {
